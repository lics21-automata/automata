...
definition "t' \<equiv> t + output_speed"
...
fun tdfa_step :: "('s, 'b) tdfa_s \<Rightarrow> ('a Al \<times> 'b Al) \<Rightarrow>
  ...
  "tdfa_step (STATE (bs, qs)) (Symb a, Symb b) = (if length bs < t' then
...
definition tdfa_Q :: "('s, 'b) tdfa_s set" where
  "tdfa_Q = {REJECT} \<union> STATE ` ({(bs :: 'b list, qs). length bs \<le> t' \<and>
...
lemma ext_qs_tdfa_Q: "STATE (bs, qs) \<in> tdfa_Q \<Longrightarrow> length bs < t' \<Longrightarrow>
...
proof -
  have "tdfa_Q \<subseteq> {ACCEPT, REJECT} \<union>
    STATE ` ({bs :: 'b list. length bs \<le> t'} \<times> {qs. qs \<subseteq> Q \<times> {0..t'}})"
    using tdfa_Q_def by fastforce
  moreover have "finite ({bs :: 'b list. length bs \<le> t'} \<times> {qs. qs \<subseteq> Q \<times> {0..t'}})"
...
lemma tdfa_inv_upd_qs: "tdfa_inv_aligned as' bs' bs b qs \<Longrightarrow>
  (case b of Blank \<Rightarrow> True | Symb b' \<Rightarrow> length bs \<ge> t') \<Longrightarrow>
  tdfa_inv (as' @ [a]) bs' bs b (upd_qs a bs qs)"
proof -
  assume assms: "tdfa_inv_aligned as' bs' bs b qs"
    "(case b of Blank \<Rightarrow> True | Symb b' \<Rightarrow> length bs \<ge> t')"
  ...
  proof (cases b)
    case (Symb b')
    have len_bs: "length bs \<ge> t + output_speed"
      using assms(2)[unfolded Symb] by (auto simp add: t'_def)
    ...
    proof (rule ccontr)
      ...
      moreover have "length cs + length cs' \<ge> length bs' + t + output_speed + 1"
        using arg_cong[OF split(3), of length] len_bs by auto
      ultimately have len_cs: "length cs \<ge> length bs' + t + 1"
        by auto
...
lemma tdfa_inv_drop_upd_qs: "tdfa_inv_aligned as' bs' bs
  (if length bs \<ge> t' then ext_b else Blank) qs \<Longrightarrow>
  ...
  (if length bs \<ge> t' then ext_b else Blank) (drop_qs m qs')"
...
next
  ...
  proof (induction bs'' arbitrary: n as bs as' bs' qs)
    ...
    have inv': "m \<le> length bs \<and> tdfa_inv_aligned (as' @ [a]) (bs' @ take m bs) (drop m bs) Blank
      ...
      by (cases "t' \<le> length bs") (auto simp: safe_hd_def)
    have q'_Q: "q' \<in> Q"
    ...
    have step: "tdfa_step (STATE (bs, qs)) (Symb a, Blank) =
      ...
      by (cases "length bs \<ge> t'") (auto simp: Let_def)
    show ?case
  ...
  next
    ...
    proof (cases "length bs \<ge> t'")
      ...
      proof -
        ...
        ultimately show ?thesis
          using True unfolding t'_def by auto
      qed
...
next
  ...
  have "\<not>t' \<le> length bs \<Longrightarrow> bs'' = []"
    using step_TF(1)
  ...
  then have "m \<le> length bs \<and>
    ...
    by (cases "t' \<le> length bs") (auto simp: safe_hd_Nil split: list.splits)
  then show ?case
...
next
  ...
  define qs' where "qs' = ext_qs b bs qs"
  define m where "m = Min (snd ` qs')"
  have props: "length bs < t'" "qs' \<noteq> {}" "q' = STATE (drop m (bs @ [b]), drop_qs m qs')"
