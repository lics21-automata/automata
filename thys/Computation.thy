...
(* an alphabet extended with a special Blank symbol *)

datatype 'a Al = Symb 'a | Blank

(* safe_hd returns the symbol from the extended alphabet under a reading head
   if the tape consists of a given list of symbols from an alphabet *)
...
(* q \<leadsto>((as, as'), (bs, bs')) q' iff the automaton has a computation from the state q
...
(* the language computed by the automaton *) 
...
(* TDFA in which at most one head moves at any step *)
...
(* simulation of a general TDFA by an oTDFA *)
...
fun lcp :: "'a list \<Rightarrow> 'a list \<Rightarrow> 'a list" where
  "lcp [] _ = []"
| "lcp _ [] = []"
| "lcp (a # as) (b # bs) = (if a = b then a # lcp as bs else [])"

fun len_lcp :: "'a list \<Rightarrow> 'a list \<Rightarrow> nat" where
  "len_lcp [] _ = 0"
| "len_lcp _ [] = 0"
| "len_lcp (a # as) (b # bs) = (if a = b then Suc (len_lcp as bs) else 0)"

lemma len_lcp: "len_lcp as bs = length (lcp as bs)"
  by (induction as bs rule: lcp.induct) auto

lemma len_lcp_le: "len_lcp as bs \<le> length as" "len_lcp as bs \<le> length bs"
  by (induction as bs rule: len_lcp.induct) auto

lemma lcp_drop_len_lcp: "lcp as bs @ drop (len_lcp as bs) as = as"
  "lcp as bs @ drop (len_lcp as bs) bs = bs"
  by (induction as bs rule: lcp.induct) auto

(* distance of two words w.r.t. their longest common prefix *)

definition lcp_dist :: "'a list \<Rightarrow> 'a list \<Rightarrow> nat" where
  "lcp_dist as bs = length as + length bs - 2 * len_lcp as bs"

lemma lcp_le_min: "len_lcp v1 v2 \<le> min (length v1) (length v2)"
  by (induction v1 v2 rule: lcp.induct) auto

lemma lcp_zero: "len_lcp v1 v2 \<ge> 0"
  by auto

lemma lcp_app_le_max: "len_lcp (v1 @ w1) (v2 @ w2) \<le> len_lcp v1 v2 + max (length w1) (length w2)"
...
lemma lcp_le_sum: "len_lcp v1 v2 \<le> length v1 + length v2"
  using lcp_le_min trans_le_add1
  by fastforce

lemma lcp_le_app: "len_lcp v1 v2 \<le> len_lcp (v1 @ w1) (v2 @ w2)"
  by (induction v1 v2 rule: lcp.induct) auto
...
lemma lcp_app_le_max_diff: "2 * len_lcp (v1 @ w1) (v2 @ w2) \<le>
...
lemma lcp_dist_same_pref[simp]: "lcp_dist (u @ v1) (u @ v2) = lcp_dist v1 v2"
  unfolding lcp_dist_def
  by (induction u) auto

value "lcp_dist [0, 0, 0] [0 :: nat]"
value "lcp_dist ([0, 0, 0] @ [0]) ([0] @ [0, 0, 0 :: nat])"
value "lcp_dist ([0, 0, 0] @ [0]) ([0] @ [1, 0, 0 :: nat])"
...
(* q \<leadsto>(as, bs) q' iff the transducer has a computation from the state q
   to the state q' reading as and producing bs *)
...
(* the language computed by the transducer *)
...
definition "bv R \<longleftrightarrow> (\<forall>k. \<exists>t. \<forall>(a1, u1) \<in> R. \<forall>(a2, u2) \<in> R.
  lcp_dist a1 a2 \<le> k \<longrightarrow> lcp_dist u1 u2 \<le> t)"

(* Lemma 8 *)

lemma "bv R \<longleftrightarrow> (\<forall>k. \<exists>t. \<forall>a b1 b2 u1 u2.
  (a @ b1, u1) \<in> R \<and> (a @ b2, u2) \<in> R \<and> length b1 + length b2 \<le> k \<longrightarrow> lcp_dist u1 u2 \<le> t)"
proof (rule iffI)
  assume bv: "bv R"
  show "(\<forall>k. \<exists>t. \<forall>a b1 b2 u1 u2.
    (a @ b1, u1) \<in> R \<and> (a @ b2, u2) \<in> R \<and> length b1 + length b2 \<le> k \<longrightarrow> lcp_dist u1 u2 \<le> t)"
  proof (rule allI)
    fix k
    obtain t where t_def: "(a1, a2) \<in> R \<Longrightarrow> (u1, u2) \<in> R \<Longrightarrow>
      lcp_dist a1 u1 \<le> k \<Longrightarrow> lcp_dist a2 u2 \<le> t" for a1 a2 u1 u2
      using bv
      by (fastforce simp: bv_def)
    have "(a @ b1, u1) \<in> R \<Longrightarrow> (a @ b2, u2) \<in> R \<Longrightarrow> length b1 + length b2 \<le> k \<Longrightarrow>
      lcp_dist b1 b2 \<le> k" for a b1 b2 u1 u2
      using lcp_dist_app_le_sum[of "[]" b1 "[]" b2]
      by auto
    then show "(\<exists>t. \<forall>a b1 b2. \<forall>u1 u2.
      (a @ b1, u1) \<in> R \<and> (a @ b2, u2) \<in> R \<and> length b1 + length b2 \<le> k \<longrightarrow> lcp_dist u1 u2 \<le> t)"
      by (auto intro!: exI[of _ t] t_def)
  qed
next
  assume bv': "(\<forall>k. \<exists>t. \<forall>a b1 b2. \<forall>u1 u2.
    (a @ b1, u1) \<in> R \<and> (a @ b2, u2) \<in> R \<and> length b1 + length b2 \<le> k \<longrightarrow> lcp_dist u1 u2 \<le> t)"
  show "bv R"
    unfolding bv_def
  proof (rule allI)
    fix k
    obtain t where t_def: "(a @ b1, u1) \<in> R \<Longrightarrow> (a @ b2, u2) \<in> R \<Longrightarrow>
      length b1 + length b2 \<le> k \<Longrightarrow> lcp_dist u1 u2 \<le> t" for a b1 u1 b2 u2
      using bv'
      by metis
    have "lcp_dist a2 u2 \<le> t" if "(a1, a2) \<in> R" "(u1, u2) \<in> R" "lcp_dist a1 u1 \<le> k"
      for a1 a2 u1 u2
      using t_def[of "lcp a1 u1" "drop (len_lcp a1 u1) a1" a2 "drop (len_lcp a1 u1) u1" u2] that
        len_lcp_le[of a1 u1]
      by (auto simp: lcp_drop_len_lcp lcp_dist_def)
    then show "\<exists>t. \<forall>(a1, a2)\<in>R. \<forall>(u1, u2)\<in>R. lcp_dist a1 u1 \<le> k \<longrightarrow> lcp_dist a2 u2 \<le> t"
...
definition "bvT \<longleftrightarrow> (\<forall>k. \<exists>t. \<forall>q1 q2 f1 f2 a b1 b2 u v1 v2 w1 w2.
...
definition "bounded t \<equiv> \<forall>q q' u v v'. q \<in> Q \<and> q' \<in> Q \<and> init \<leadsto>(u, v @ v') q \<and> active q [] \<and>
  init \<leadsto>(u, v) q' \<and> active q' v' \<longrightarrow> length v' \<le> t"

(* Definition 11 *)

lemma bounded_alt: "bounded t \<longleftrightarrow> (\<forall>q1 q2 f1 f2 a b1 b2 u v w1 w2.
  q1 \<in> Q \<and> q2 \<in> Q \<and> accept f1 \<and> accept f2 \<and>
  init \<leadsto>(a, u @ v) q1 \<and> q1 \<leadsto>(b1, w1) f1 \<and>
  init \<leadsto>(a, u) q2 \<and> q2 \<leadsto>(b2, v @ w2) f2 \<longrightarrow> length v \<le> t)"
  by (fastforce simp: bounded_def active_def)
...
(* computation relation tracking all intermediate steps in a computation *)
...
(* Definition 5; note that a state q is useful iff active q [] holds *)
...
lemma bounded_dest: "bounded K \<Longrightarrow> init \<leadsto>(u, v @ v') q \<Longrightarrow> active q [] \<Longrightarrow>
  init \<leadsto>(u, v) q' \<Longrightarrow> active q' v' \<Longrightarrow> length v' \<le> K"
  using comp_closed[OF _ init_in_Q]
  by (fastforce simp add: bounded_def)

end

(* NFT with bounded trailing with trailing bound t *)
